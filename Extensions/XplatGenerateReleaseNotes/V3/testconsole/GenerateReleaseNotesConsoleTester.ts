...
async function run(): Promise<number> {
  var promise = new Promise<number>(async (resolve, reject) => {
    try {
      ...
      if (showUsage) {
        console.error(
          "USAGE: node GenerateReleaseNotesConsoleTester.js --filename settings.json --pat <Azure-DevOps-PAT> --githubpat <Optional GitHub-PAT> --bitbucketuser <Optional Bitbucket User> --bitbucketsecret <Optional Bitbucket App Secret>"
        );
      } else {
        ...
        if (fs.existsSync(filename)) {
          ...
          var getParentsAndChildren = getBoolean(
            settings.getParentsAndChildren
          );
          var getAllParents = getBoolean(settings.getAllParents);
          var searchCrossProjectForPRs = getBoolean(
            settings.searchCrossProjectForPRs
          );

          var stopOnRedeploy = settings.stopOnRedeploy;
          var sortWi = getBoolean(settings.SortWi);

          var customHandlebarsExtensionCode =
            settings.customHandlebarsExtensionCode;
          if (fs.existsSync(settings.customHandlebarsExtensionCode)) {
            console.log(
              "customHandlebarsExtensionCode value specifies a file, so loading string from file"
            );
            customHandlebarsExtensionCode = fs.readFileSync(
              settings.customHandlebarsExtensionCode
            );
          }

          var customHandlebarsExtensionFile =
            settings.customHandlebarsExtensionFile;
          var customHandlebarsExtensionFolder =
            settings.customHandlebarsExtensionFolder;
          ...
          var getIndirectPullRequests = getBoolean(
            settings.getIndirectPullRequests
          );
          var repositoryId = settings.repositoryId;

...
async function run(): Promise<number> {
  var promise = new Promise<number>(async (resolve, reject) => {
    try {
      ...
      var searchCrossProjectForPRs = tl.getBoolInput(
      ...
      var customHandlebarsExtensionCode = tl.getInput(
        "customHandlebarsExtensionCode"
      );
      var customHandlebarsExtensionFile = tl.getInput(
        "customHandlebarsExtensionFile"
      );
      var customHandlebarsExtensionFolder = tl.getInput(
...
run()
  .then((result) => {
    if (result === -1) {
      tl.setResult(
        ...
        "Skipped release notes generation as redeploy"
      );
    } else {
  ...
  .catch((err) => {
    agentApi.publishEvent("reliability", {
      issueType: "error",

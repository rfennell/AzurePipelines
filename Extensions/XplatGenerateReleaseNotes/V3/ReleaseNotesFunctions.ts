...
export function getDeploymentCount(
    ...
    if (environment.name.toLowerCase() === environmentName) {
      ...
      var currentDeployment =
        environment.preDeployApprovals[
          environment.preDeployApprovals.length - 1
        ];
  ...
  if (attemptCount === 0) {
    ...
    agentApi.logInfo(
      `Identified [${environmentName}] as having deployment count of [${attemptCount}]`
    );
  }
  return attemptCount;
}

export function getReleaseDefinitionId(
  environments: ReleaseEnvironment[],
  environmentName: string
): number {
  ...
  agentApi.logInfo(
    `Identified [${environmentName}] as having id [${environmentId}]`
  );
  return environmentId;
}

export function getSimpleArtifactArray(
  artifacts: Artifact[]
): SimpleArtifact[] {
  ...
  return new Promise<GitPullRequest[]>(async (resolve, reject) => {
    ...
    try {
      ...
      do {
        ...
        var prListBatch = await gitApi.getPullRequestsByProject(
          projectName,
          filter,
          0,
          skip,
...
export async function getMostRecentSuccessfulDeployment(
  ...
  return new Promise<Deployment>(async (resolve, reject) => {
    ...
    try {
      ...
      var successfulDeployments = await releaseApi
        .getDeployments(
          teamProject,
          releaseDefinitionId,
          environmentId,
      ...
      if (successfulDeployments && successfulDeployments.length > 0) {
        agentApi.logInfo(
          `Found ${successfulDeployments.length} successful releases`
        );
        if (
          overrideBuildReleaseId &&
          !isNaN(parseInt(overrideBuildReleaseId))
        ) {
          agentApi.logInfo(
            `Trying to find successful deployment with the override release ID of '${overrideBuildReleaseId}'`
          );
          mostRecentDeployment = successfulDeployments.find(
            (element) => element.release.id === parseInt(overrideBuildReleaseId)
          );
          if (mostRecentDeployment) {
            agentApi.logInfo(
              `Found matching override release ${mostRecentDeployment.release.name}`
            );
          } else {
          ...
          agentApi.logInfo(
            `Finding the last successful release ${mostRecentDeployment.release.name}`
          );
        }
      } else {
...
export async function expandTruncatedCommitMessages(
  restClient: WebApi,
  globalCommits: Change[],
  gitHubPat: string,
        ...
        try {
          ...
          if (change.location.startsWith("https://api.github.com/")) {
            ...
            if (gitHubRes.statusCode === 200) {
              ...
              agentApi.logWarn(
                `Cannot access API ${gitHubRes.statusCode} accessing ${change.location}`
              );
              agentApi.logWarn(
                `The most common reason for this failure is that the GitHub Repo is private and a Personal Access Token giving read access needs to be passed as a parameter to this task`
              );
            }
            ...
            if (
              bitbucketUser &&
              bitbucketUser.length > 0 &&
              bitbucketSecret &&
              bitbucketSecret.length > 0
            ) {
              let auth = new BasicCredentialHandler(
                bitbucketUser,
                bitbucketSecret
              );
              ...
              agentApi.logInfo(
                `No Bitbucket user and app secret passed so cannot access private Bitbucket repos`
              );
            }
            ...
            if (bitbucketRes.statusCode === 200) {
              ...
              agentApi.logWarn(
                `Cannot access API ${bitbucketRes.statusCode} accessing ${change.location}`
              );
              agentApi.logWarn(
                `The most common reason for this failure is that the Bitbucket Repo is private and a Personal Access Token giving read access needs to be passed as a parameter to this task`
              );
            }
          } else {
            ...
            if (vstsRes.statusCode === 200) {
              ...
              agentApi.logWarn(
                `Cannot access API ${vstsRes.statusCode} accessing ${change.location}`
              );
              agentApi.logWarn(
                `The most common reason for this failure is that the account defined by the agent access token does not  have rights to read the required repo`
              );
            }
          }
        } catch (err) {
    ...
    try {
      for (let prIndex = 0; prIndex < pullRequests.length; prIndex++) {
        ...
        if (!prDetails.labels || prDetails.labels.length === 0) {
          ...
          const prLabels = await gitApi.getPullRequestLabels(
            prDetails.repository.id,
            prDetails.pullRequestId
          );
          prDetails.labels = prLabels;
        }
        // and added the WI IDs
        var wiRefs = await gitApi.getPullRequestWorkItemRefs(
          prDetails.repository.id,
          prDetails.pullRequestId
        );
        ...
        agentApi.logDebug(
          `Added ${prDetails.associatedWorkitems.length} work items for ${prDetails.pullRequestId}`
        );

        prDetails.associatedCommits = [];
        var csRefs = await gitApi.getPullRequestCommits(
          prDetails.repository.id,
          prDetails.pullRequestId
        );
        for (let csIndex = 0; csIndex < csRefs.length; csIndex++) {
          prDetails.associatedCommits.push(
            await gitApi.getCommit(
              csRefs[csIndex].commitId,
              prDetails.repository.id
            )
          );
        }
        agentApi.logDebug(
          `Added ${prDetails.associatedCommits.length} commits for ${prDetails.pullRequestId}, note this includes commits on the PR source branch not associated directly with the build)`
        );
      }
  ...
  return new Promise<Change[]>(async (resolve, reject) => {
    try {
      ...
      for (let index = 0; index < changes.length; index++) {
        ...
        try {
          agentApi.logInfo(
            `Enriched change ${change.id} of type ${change.type}`
          );
          if (change.type === "TfsGit") {
            ...
            if (repositoryId !== "") {
              ...
              if (urlParts.host === "dev.azure.com") {
                ...
                agentApi.logInfo(
                  `Cannot enriched as location URL not in dev.azure.com format`
                );
              }
            }
            let gitDetails = await gitApi.getChanges(change.id, repoId);
            agentApi.logInfo(
              `Enriched with details of ${gitDetails.changes.length} files`
            );
            extraDetail = gitDetails.changes;
          } else if (change.type === "TfsVersionControl") {
            var tfvcDetail = await tfvcApi.getChangesetChanges(
              parseInt(change.id.substring(1))
            );
            agentApi.logInfo(
              `Enriched with details of ${tfvcDetail.length} files`
            );
            ...
            if (gitHubRes.statusCode === 200) {
              ...
              agentApi.logInfo(
                `Enriched with details of ${gitHubFiles.length} files`
              );
            ...
            agentApi.logWarn(
              `This task does not currently support getting file details associated to a commit on Bitbucket`
            );
          } else {
            agentApi.logWarn(
              `Cannot preform enrichment as type ${change.type} is not supported for enrichment`
            );
          }
...
export function getCredentialHandler(pat: string): IRequestHandler {
  if (!pat || pat.length === 0) {
    ...
    agentApi.logInfo(
      "Creating the credential handler using override PAT (suitable for local testing or if the OAUTH token cannot be used)"
    );
...
export async function getTestsForBuild(
  ...
  return new Promise<TestCaseResult[]>(async (resolve, reject) => {
    ...
    try {
      let builtTestResults = await testAPI.getTestResultsByBuild(
        teamProject,
        buildId
      );
      if (builtTestResults.length > 0) {
        for (let index = 0; index < builtTestResults.length; index++) {
          ...
          if (
            testList.filter((e) => e.testRun.id === `${test.runId}`).length ===
            0
          ) {
            ...
            tl.debug(
              `Skipping adding tests for test run ${test.runId} as already added`
            );
          }
        }
      } else {
  ...
  if (newArray.length > 0) {
    newArray.forEach((test) => {
      if (
        masterArray.filter(
          (e) =>
            e.testCaseReferenceId === test.testCaseReferenceId &&
            e.testRun.id === test.testRun.id
        ).length === 0
      ) {
        tl.debug(
          `Adding the test case ${test.testCaseReferenceId} for test run ${test.testRun.id} as not present in the master list`
        );
        masterArray.push(test);
      } else {
        tl.debug(
          `Skipping the test case ${test.testCaseReferenceId} for test run ${test.testRun.id} as already present in the master list`
        );
      }
    });
  }
  tl.debug(
    `The updated global test array contains ${masterArray.length} items`
  );
  return masterArray;
}
...
export async function getTestsForRelease(
  ...
  return new Promise<TestCaseResult[]>(async (resolve, reject) => {
    ...
    try {
      for (
        let envIndex = 0;
        envIndex < release.environments.length;
        envIndex++
      ) {
        const env = release.environments[envIndex];
        let envTestResults = await testAPI.getTestResultDetailsForRelease(
          teamProject,
          release.id,
          env.id
        );
        if (envTestResults.resultsForGroup.length > 0) {
          for (
            let index = 0;
            index < envTestResults.resultsForGroup[0].results.length;
            index++
          ) {
            const test = envTestResults.resultsForGroup[0].results[index];
            if (
              testList.filter((e) => e.testRun.id === `${test.testRun.id}`)
                .length === 0
            ) {
              tl.debug(`Adding tests for test run ${test.testRun.id}`);
              let run = await testAPI.getTestResults(
                teamProject,
                parseInt(test.testRun.id)
              );
              testList.push(...run);
            } else {
              tl.debug(
                `Skipping adding tests for test run ${test.testRun.id} as already added`
              );
            }
          }
        } else {
          tl.debug(
            `No tests associated with release ${release.id} environment ${env.name}`
          );
        }
      }
      resolve(testList);
...
export function getTemplate(
  ...
  if (template.includes(handlebarIndicator)) {
    ...
    agentApi.logError(
      "The template is not on handlebars format, load template has been skipped"
    );
    template = "";
  }
  return template;
      ...
      if (
        ...
        if (!relatedWorkItems.find((element) => element.id === id)) {
          ...
          relatedWorkItems.push(
            await workItemTrackingApi.getWorkItem(
              id,
              null,
              null,
              WorkItemExpand.All,
...
export async function getAllParentWorkitems(
      ...
      for (let relIndex = 0; relIndex < wi.relations.length; relIndex++) {
        ...
        if (relation.attributes.name === "Parent") {
          ...
          if (!allRelatedWorkItems.find((element) => element.id === id)) {
            ...
            agentApi.logInfo(
              `Skipping ${id} as already in the found parent list`
            );
          }
        }
      }
  ...
  agentApi.logInfo(
    `Added ${
      allRelatedWorkItems.length - knownWI
    } parent WI to the list of direct relations`
  );
...
export async function getFullWorkItemDetails(
  ...
  if (workItemIds && workItemIds.length > 0) {
    ...
    while (indexEnd <= workItemIds.length && indexStart !== indexEnd) {
      ...
      agentApi.logInfo(
        `Getting full details of WI batch from index: [${indexStart}] to [${indexEnd}]`
      );
      var subListDetails = await workItemTrackingApi.getWorkItems(
        subList,
        null,
        null,
        WorkItemExpand.All,
      ...
      indexStart = indexEnd;
      indexEnd =
        workItemIds.length - indexEnd > 200
          ? indexEnd + 200
          : workItemIds.length;
    }
  }
...
export function processTemplate(
  ...
  if (template.length > 0) {
    ...
    agentApi.logDebug(
      `  Indirect PR: ${inDirectlyAssociatedPullRequests.length}`
    );
    ...
    // add our helper to find PR
    handlebars.registerHelper("lookup_a_pullrequest_by_merge_commit", function (
      array,
      commitId
    ) {
      return array.find(
        (element) => element.lastMergeCommit.commitId === commitId
      );
    });

    if (
      typeof customHandlebarsExtensionCode !== undefined &&
      customHandlebarsExtensionCode &&
      customHandlebarsExtensionCode.length > 0
    ) {
      agentApi.logDebug(
        `Saving custom Handlebars code to file in folder ${customHandlebarsExtensionFolder}`
      );

      if (
        !customHandlebarsExtensionFolder ||
        customHandlebarsExtensionFolder.length === 0
      ) {
      ...
      writeFile(
        `${customHandlebarsExtensionFolder}/${customHandlebarsExtensionFile}.js`,
        customHandlebarsExtensionCode,
        true,
        false
      );
...
export async function getLastSuccessfulBuildByStage(
  ...
  if (builds.length > 1) {
    ...
    if (overrideBuildReleaseId && overrideBuildReleaseId.length > 0) {
      ...
      var overrideBuild = builds.find(
        (element) => element.id.toString() === overrideBuildReleaseId
      );
      if (overrideBuild) {
        ...
        let record = timeline.records.find(
        ...
        agentApi.logError(
          `There is no build matching the override ID of ${overrideBuildReleaseId}`
        );
        return;
      }
    }
    ...
    for (let buildIndex = 0; buildIndex < builds.length; buildIndex++) {
      ...
      if (build.id.toString() === buildId.toString()) {
        ...
        if (
          tags.length === 0 ||
          (tags.length > 0 &&
            build.tags.sort().join(",") === tags.sort().join(","))
        ) {
          ...
          if (timeline && timeline.records) {
            for (
              let timelineIndex = 0;
              timelineIndex < timeline.records.length;
              timelineIndex++
            ) {
              ...
              if (record.type === "Stage") {
                if (
                  (record.name === stageName ||
                    record.identifier === stageName) &&
                  (record.state.toString() === "2" ||
                    record.state.toString() === "completed") && // completed
                  (record.result.toString() === "0" ||
                    record.result.toString() === "succeeded")
                ) {
                  // succeeded
                  agentApi.logInfo(
                    `Found required stage ${record.name} in the completed and successful state in build ${build.id}`
                  );
                  return {
                    id: build.id,
                    stage: record,
            ...
            agentApi.logInfo(
              "Skipping check as no timeline available for this build"
            );
          }
        } else {
  ...
  return new Promise<number>(async (resolve, reject) => {
    ...
    try {
      if (releaseId === undefined || !releaseId) {
        ...
        if (!currentBuild) {
          agentApi.logError(
            `Unable to locate the current build with id ${buildId} in the project ${teamProject}`
          );
          reject(-1);
          return;
        }
        ...
        if (checkStage) {
          ...
          if (tags && tags.length > 0) {
            tagArray = tags.split(",");
            agentApi.logInfo(
              `Only considering builds with the tag(s) '${tags}'`
            );
          }
          if (overrideStageName && overrideStageName.length > 0) {
            agentApi.logInfo(
              `Overriding current stage '${stageName}' with '${overrideStageName}'`
            );
          ...
          if (overrideBuildReleaseId && overrideBuildReleaseId.length > 0) {
            if (isNaN(parseInt(overrideBuildReleaseId, 10))) {
              agentApi.logError(
                `The override build ID '${overrideBuildReleaseId}' is not a number `
              );
              resolve(-1);
              return;
            }
            agentApi.logInfo(
              `Using the override for the last successful build of ID '${overrideBuildReleaseId}'`
            );
          }

          agentApi.logInfo(
            `Getting items associated the builds since the last successful build to the stage '${stageName}'`
          );
          ...
          if (lastGoodBuildId !== 0) {
            ...
            mostRecentSuccessfulBuild = await buildApi.getBuild(
              teamProject,
              lastGoodBuildId
            );
            ...
            if (mostRecentSuccessfulBuild.repository.type === "TfsGit") {
              ...
              let commitInfo = await issue349.getCommitsAndWorkItemsForGitRepo(
                organisation,
                mostRecentSuccessfulBuild.sourceVersion,
                currentBuild.sourceVersion,
                currentBuild.repository.id
              );
              globalCommits = commitInfo.commits;
              globalWorkItems = commitInfo.workItems;
            } else {
              // Fall back to original behavior
              globalCommits = await buildApi.getChangesBetweenBuilds(
                teamProject,
                lastGoodBuildId,
                buildId
              );
              globalWorkItems = await buildApi.getWorkItemsBetweenBuilds(
                teamProject,
                lastGoodBuildId,
                buildId
              );
            ...
            console.log(
              "There has been no past successful build for this stage, so we can just get details from this build"
            );
            globalCommits = await buildApi.getBuildChanges(
              teamProject,
              buildId
            );
            globalWorkItems = await buildApi.getBuildWorkItemsRefs(
              teamProject,
              buildId
            );
          }
        ...
        globalCommits = await enrichChangesWithFileDetails(
          gitApi,
          tfvcApi,
          globalCommits,
          gitHubPat,
        ...
        agentApi.logInfo(
          `Show associated items for primary artifact only is set to ${showOnlyPrimary}`
        );

        // check of redeploy
        if (stopOnRedeploy === true) {
          if (
            getDeploymentCount(currentRelease.environments, environmentName) > 1
          ) {
            agentApi.logWarn(
              `Skipping release note generation as this deploy is a re-deployment`
            );
            resolve(-1);
            return;
          }
        ...
        if (!currentRelease) {
          agentApi.logError(
            `Unable to locate the current release with id ${releaseId}`
          );
          resolve(-1);
          return;
        }

        var environmentId = getReleaseDefinitionId(
          currentRelease.environments,
          environmentName
        );
        ...
        if (overrideBuildReleaseId && overrideBuildReleaseId.length > 0) {
          if (isNaN(parseInt(overrideBuildReleaseId, 10))) {
            agentApi.logError(
              `The override release ID '${overrideBuildReleaseId}' is not a number `
            );
            resolve(-1);
            return;
          } else {
            agentApi.logInfo(
              `Using the override for the last successful release of ID '${overrideBuildReleaseId}'`
            );
          }
        }
        ...
        var arifactsInThisRelease = getSimpleArtifactArray(
        ...
        if (mostRecentSuccessfulDeployment) {
          ...
          mostRecentSuccessfulDeploymentRelease = await releaseApi.getRelease(
            teamProject,
            mostRecentSuccessfulDeployment.release.id
          );
          agentApi.logInfo(
            `Getting all artifacts in the most recent successful release [${mostRecentSuccessfulDeployment.release.name}]...`
          );
          arifactsInMostRecentRelease = getSimpleArtifactArray(
            mostRecentSuccessfulDeployment.release.artifacts
          );
          mostRecentSuccessfulDeploymentName =
            mostRecentSuccessfulDeployment.release.name;
          agentApi.logInfo(`Found ${arifactsInMostRecentRelease.length}`);
        } else {
          agentApi.logInfo(
            `Skipping fetching artifact in the most recent successful release as there isn't one.`
          );
        ...
        for (var artifactInThisRelease of arifactsInThisRelease) {
          agentApi.logInfo(
            `Looking at artifact [${artifactInThisRelease.artifactAlias}]`
          );
          agentApi.logInfo(
            `Artifact type [${artifactInThisRelease.artifactType}]`
          );
          agentApi.logInfo(
            `Build Definition ID [${artifactInThisRelease.buildDefinitionId}]`
          );
          agentApi.logInfo(
            `Build Number: [${artifactInThisRelease.buildNumber}]`
          );
          ...
          if (
            ...
            (showOnlyPrimary === true &&
              artifactInThisRelease.isPrimary === true)
          ) {
            if (arifactsInMostRecentRelease.length > 0) {
              if (artifactInThisRelease.artifactType === "Build") {
                agentApi.logInfo(
                  `Looking for the [${artifactInThisRelease.artifactAlias}] in the most recent successful release [${mostRecentSuccessfulDeploymentName}]`
                );
                for (var artifactInMostRecentRelease of arifactsInMostRecentRelease) {
                  if (
                    artifactInThisRelease.artifactAlias.toLowerCase() ===
                    artifactInMostRecentRelease.artifactAlias.toLowerCase()
                  ) {
                    agentApi.logInfo(
                      `Found artifact [${artifactInMostRecentRelease.artifactAlias}] with build number [${artifactInMostRecentRelease.buildNumber}] in release [${mostRecentSuccessfulDeploymentName}]`
                    );
                    ...
                    if (isInitialRelease) {
                      agentApi.logInfo(
                        `This is the first release so checking what commits and workitems are associated with artifacts`
                      );
                      var builds = await buildApi.getBuilds(
                        artifactInThisRelease.sourceId,
                        [parseInt(artifactInThisRelease.buildDefinitionId)]
                      );
                      commits = [];
                      workitems = [];
                      ...
                      for (var build of builds) {
                        try {
                          agentApi.logInfo(
                            `Getting the details of build ${build.id}`
                          );
                          var buildCommits = await buildApi.getBuildChanges(
                            teamProject,
                            build.id
                          );
                          commits.push(...buildCommits);
                          var buildWorkitems = await buildApi.getBuildWorkItemsRefs(
                            teamProject,
                            build.id
                          );
                          ...
                          agentApi.logWarn(
                            `There was a problem getting the details of the build ${err}`
                          );
                        }
                      }
                    } else if (
                      artifactInMostRecentRelease.buildId !==
                      artifactInThisRelease.buildId
                    ) {
                      agentApi.logInfo(
                        `Checking what commits and workitems have changed from [${artifactInMostRecentRelease.buildNumber}][ID ${artifactInMostRecentRelease.buildId}] => [${artifactInThisRelease.buildNumber}] [ID ${artifactInThisRelease.buildId}]`
                      );
                      ...
                      try {
                        ...
                        if (!activateFix) {
                          agentApi.logInfo(
                            "Defaulting on the workaround for build API limitation (see issue #349 set 'ReleaseNotes.Fix349=false' to disable)"
                          );
                          activateFix = "true";
                        }
                        ...
                        if (
                          activateFix &&
                          activateFix.toLowerCase() === "true"
                        ) {
                          let baseBuild = await buildApi.getBuild(
                            artifactInThisRelease.sourceId,
                            parseInt(artifactInMostRecentRelease.buildId)
                          );
                          agentApi.logInfo(
                            "Using workaround for build API limitation (see issue #349)"
                          );
                          ...
                          if (baseBuild.repository.type === "TfsGit") {
                            let currentBuild = await buildApi.getBuild(
                              artifactInThisRelease.sourceId,
                              parseInt(artifactInThisRelease.buildId)
                            );
                            let commitInfo = await issue349.getCommitsAndWorkItemsForGitRepo(
                              organisation,
                              baseBuild.sourceVersion,
                              currentBuild.sourceVersion,
                              currentBuild.repository.id
                            );
                            ...
                            // Fall back to original behavior
                            commits = await buildApi.getChangesBetweenBuilds(
                              artifactInThisRelease.sourceId,
                              parseInt(artifactInMostRecentRelease.buildId),
                              parseInt(artifactInThisRelease.buildId),
                              5000
                            );
                            workitems = await buildApi.getWorkItemsBetweenBuilds(
                              artifactInThisRelease.sourceId,
                              parseInt(artifactInMostRecentRelease.buildId),
                              parseInt(artifactInThisRelease.buildId),
                              5000
                        ...
                        agentApi.logWarn(
                          `There was a problem getting the details of the CS/WI for the build ${err}`
                        );
                      }
                    } else {
                      commits = [];
                      workitems = [];
                      agentApi.logInfo(
                        `Build for artifact [${artifactInThisRelease.artifactAlias}] has not changed.  Nothing to do`
                      );
                    }

                    // look for any test in the current build
                    agentApi.logInfo(
                      `Getting test associated with the latest build [${artifactInThisRelease.buildId}]`
                    );
                    tests = await getTestsForBuild(
                      testApi,
                      teamProject,
                      parseInt(artifactInThisRelease.buildId)
                    );
                    ...
                    if (tests) {
                      agentApi.logInfo(
                        `Found ${tests.length} test associated with the build [${artifactInThisRelease.buildId}] adding any not already in the global test list to the list`
                      );
                    ...
                    let artifact = await buildApi.getBuild(
                      artifactInThisRelease.sourceId,
                      parseInt(artifactInThisRelease.buildId)
                    );
                    agentApi.logInfo(
                      `Adding the build [${artifact.id}] and its association to the unified results object`
                    );
                    let fullBuildWorkItems = await getFullWorkItemDetails(
                      workItemTrackingApi,
                      workitems
                    );
                    globalBuilds.push(
                      new UnifiedArtifactDetails(
                        artifact,
                        commits,
                        fullBuildWorkItems,
                        tests
                    ...
                    agentApi.logInfo(
                      `Detected ${commits.length} commits/changesets and ${workitems.length} workitems between the current build and the last successful one`
                    );
                    agentApi.logInfo(
                      `Detected ${tests.length} tests associated within the current build.`
                    );
                  }
                }
              } else {
                agentApi.logInfo(
                  `Skipping artifact as cannot get WI and commits/changesets details`
                );
              }
            }
          } else {
            agentApi.logInfo(
              `Skipping artifact as only primary artifact required`
            );
          }
          agentApi.logInfo(``);
        }
        ...
        releaseTests = await getTestsForRelease(
          testApi,
          teamProject,
          currentRelease
        );
      ...
      let expandedGlobalCommits = await expandTruncatedCommitMessages(
        organisation,
        globalCommits,
        gitHubPat,
        bitbucketUser,
      ...
      if (
        !expandedGlobalCommits ||
        expandedGlobalCommits.length !== globalCommits.length
      ) {
      ...
      let fullWorkItems = await getFullWorkItemDetails(
        workItemTrackingApi,
        globalWorkItems
      );
      ...
      if (getParentsAndChildren) {
        ...
        relatedWorkItems = await getAllDirectRelatedWorkitems(
          workItemTrackingApi,
          fullWorkItems
        );
      }
      ...
      if (getAllParents) {
        ...
        relatedWorkItems = await getAllParentWorkitems(
          workItemTrackingApi,
          relatedWorkItems
        );
      }
      ...
      if (sortWi) {
        ...
        fullWorkItems = fullWorkItems.sort((a, b) =>
          a.fields["System.WorkItemType"] > b.fields["System.WorkItemType"]
            ? 1
            : a.fields["System.WorkItemType"] ===
              b.fields["System.WorkItemType"]
            ? a.id > b.id
              ? 1
              : -1
            : -1
      ...
      try {
        if (isNaN(buildId)) {
          // only try this if we have numeric build ID, not a GUID see #694
          agentApi.logInfo(
            `Do not have an Azure DevOps numeric buildId, so skipping trying to get  any build PR trigger info`
          );
        } else {
          agentApi.logDebug(
            `Getting the details of build ${buildId} from default project`
          );
          currentBuild = await buildApi.getBuild(teamProject, buildId);
          // and enhance the details if they can
          if (
            currentBuild.repository.type === "TfsGit" &&
            currentBuild.triggerInfo["pr.number"]
          ) {
            agentApi.logInfo(
              `The default artifact for the build/release was triggered by the PR ${currentBuild.triggerInfo["pr.number"]}, getting details`
            );
            prDetails = await gitApi.getPullRequestById(
              parseInt(currentBuild.triggerInfo["pr.number"])
            );
        ...
        agentApi.logWarn(
          `Could not get details of Trigger PR an error was seen: ${error}`
        );
      }
      ...
      if (searchCrossProjectForPRs) {
        agentApi.logInfo(
          `Getting all completed Azure DevOps Git Repo PRs in the Organisation`
        );
      } else {
        agentApi.logInfo(
          `Getting all completed Azure DevOps Git Repo PRs in the Team Project ${teamProject}`
        );
        prProjectFilter = teamProject;
      }

      try {
        var allPullRequests: GitPullRequest[] = await getPullRequests(
          gitApi,
          prProjectFilter
        );
        if (allPullRequests && allPullRequests.length > 0) {
          agentApi.logInfo(
            `Found ${allPullRequests.length} Azure DevOps PRs in the repo`
          );
          globalCommits.forEach((commit) => {
            if (commit.type === "TfsGit") {
              agentApi.logInfo(
                `Checking for PRs associated with the commit ${commit.id}`
              );
              ...
              allPullRequests.forEach((pr) => {
                if (pr.lastMergeCommit) {
                  if (pr.lastMergeCommit.commitId === commit.id) {
                    agentApi.logInfo(
                      `- PR ${pr.pullRequestId} matches the commit ${commit.id}`
                    );
                    globalPullRequests.push(<EnrichedGitPullRequest>pr);
                  }
                } else {
                  agentApi.logInfo(
                    `- PR ${pr.pullRequestId} does not have a lastMergeCommit`
                  );
                }
              });
            } else {
              agentApi.logDebug(
                `Cannot check for associated PR as the commit ${commit.id} is not in an Azure DevOps repo`
              );
            }
          });
        } else {
        ...
        agentApi.logWarn(
          `Could not get details of any PR an error was seen: ${error}`
        );
      }

      // remove duplicates
      globalPullRequests = globalPullRequests.filter(
        (thing, index, self) =>
          index ===
          self.findIndex((t) => t.pullRequestId === thing.pullRequestId)
      );
      ...
      if (getIndirectPullRequests === true) {
        ...
        if (allPullRequests && allPullRequests.length > 0) {
          for (
            let prIndex = 0;
            prIndex < globalPullRequests.length;
            prIndex++
          ) {
            const pr = globalPullRequests[prIndex];
            for (
              let csIndex = 0;
              csIndex < pr.associatedCommits.length;
              csIndex++
            ) {
              const cs = pr.associatedCommits[csIndex];
              var foundPR = allPullRequests.find(
                (e) => e.lastMergeCommit.commitId === cs.commitId
              );
              if (foundPR) {
                agentApi.logInfo(
                  `Found the PR ${foundPR.pullRequestId} associated wth ${cs.commitId} added to the 'inDirectlyAssociatedPullRequests' array`
                );
                inDirectlyAssociatedPullRequests.push(
                  <EnrichedGitPullRequest>foundPR
                );
              }
            }
          }
        ...
        inDirectlyAssociatedPullRequests = await enrichPullRequest(
          gitApi,
      ...
      agentApi.logInfo(
        `Total Related Workitems (Parent/Children): [${relatedWorkItems.length}]`
      );
      ...
      agentApi.logInfo(
        `Total Indirect Pull Requests: [${inDirectlyAssociatedPullRequests.length}]`
      );
